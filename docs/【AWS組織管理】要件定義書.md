承知いたしました。ご提案させていただいた内容を元の要件定義書に反映し、更新しました。
...
| **入力** | 管理者による構成変更の適用。 |
| **処理** | 1. 構成変更の適用により、AWS Organizations APIを通じて新しいメンバーアカウントを作成する。<br>2. 作成されたアカウントを、用途に応じた適切なOU（例: `Workloads-Dev`, `Workloads-Prd`）に自動的に配置する。 |
...
| **処理** | 1. **リージョン制限:** 東京 (`ap-northeast-1`) 以外のリージョンでのリソース作成を、全アカウントで禁止する。但し、東京リージョン以外にしか存在しないサービスを利用する場合は、例外として他リージョンを許容する。<br>2. **特権操作制限:** ルートユーザーによるAPIアクセスをブロックし、誤操作や不正利用のリスクを低減する。<br>3. **セキュリティ設定保護:** 管理アカウント以外の全アカウントにおいて、本管理基盤が設定したIAMロールの削除などの、ガバナンス上重要な設定の変更を禁止する。 |
| **出力** | ポリシーに違反するAPIリクエストに対するDeny（拒否）応答。 |

**#### F-ORG-003: 発見的ガードレール機能**

| 項目 | 内容 |
| :--- | :--- |
| **概要** | AWS IAM Access AnalyzerやAWS Trusted Advisorなどを活用し、セキュリティ上のリスクやベストプラクティスからの逸脱を継続的に監視する。 |
| **入力** | 各AWSアカウントのリソース設定、IAMポリシーなど。 |
| **処理** | 1. 意図しない外部公開の検知 (IAM Access Analyzer): 組織内のS3バケットやIAMロールなどが、意図せず外部のエンティティに公開されていないかを継続的にスキャンし、検出結果（結果）を管理者に通知する。<br>2. ベストプラクティス逸脱の検知 (Trusted Advisor): セキュリティグループの不適切なポート開放やMFAが設定されていないルートアカウントなど、AWSの基本的なベストプラクティスに違反している項目を全アカウント横断で検知し、ダッシュボードに表示する。 |
| **出力** | IAM Access Analyzerの検出結果、Trusted Advisorのチェック結果。 |
...
| **処理** | 1. 管理者はIAM Identity Center上でユーザーとグループを定義する。<br>2. 「管理者」「開発者」といった権限セット（Permission Set）を作成し、各グループに割り当てる。<br>3. ユーザーはSSOポータルに一度サインインするだけで、自身にアクセスが許可されたAWSアカウントとロールの一覧を閲覧し、選択してコンソールにログインできる。<br>4. 各メンバーアカウントにはIAMユーザーを作成せず、SSOによるアクセスを原則とする。 |
