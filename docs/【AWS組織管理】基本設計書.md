...
- [3. 機能設計](#3-機能設計)
  - [3.1. 発見的ガードレール機能 (F-ACC-001)](#31-発見的ガードレール機能-f-acc-001)
  - [3.2. コスト監視・通知機能 (F-ACC-002)](#32-コスト監視・通知機能-f-acc-002)
  - [3.3. シングルサインオン (SSO) 機能 (F-ACC-003)](#33-シングルサインオン-sso-機能-f-acc-003)
  - [3.4. 監査ログ収集機能 (F-ACC-004)](#34-監査ログ収集機能-f-acc-004)
- [4. データ設計](#4-データ設計)
...
- **処理概要:** AWS IAM Access AnalyzerとAWS Trusted Advisorを本番アカウント単体で有効化し、リスクや逸脱を継続的に監視・通知する。
- **トリガー:** 各AWSサービスによる継続的な自動スキャン、および定期的なチェック。
- **入力:** 本番アカウントのリソース構成情報。
- **処理シーケンス:**
    1. **IAM Access Analyzer:**
        - Terraformを用いて、本番アカウント単体でアカウントタイプのAnalyzerを有効化する。
        - 意図しない外部アクセスを許可するリソースを検知すると、Findingを生成する。
        - EventBridgeルールでSNS通知を行い、管理者へメール送信する。
    2. **Trusted Advisor:**
        - 本番アカウント単体でチェック結果を監視する。

### 3.2. コスト監視・通知機能 (F-ACC-002)

- **処理概要:** AWS Budgetsを用いて本番アカウントのコストを監視し、予算超過（または予測超過）時に通知を行う。
- **処理シーケンス:**
    1. Terraformで予算額（Budget）と通知先（SNS/Email）を定義する。
    2. コストが設定した閾値（例：予算の80%）に達した場合、SNS経由で管理者に通知する。

### 3.3. シングルサインオン (SSO) 機能 (F-ACC-003)

- **処理概要:** AWS IAM Identity Center（アカウントインスタンス）を用いて、AWS Organizationsに依存せず、本番アカウント単体へのアクセスを管理する。
- **処理シーケンス:**
    1. 本番アカウント上でIAM Identity Centerをアカウントインスタンスとして有効化する。
    2. ユーザー、グループ、および許可セット（Permission Set）を定義する。
    3. 本番アカウントに対し、グループまたはユーザーと許可セットを紐付ける。
...
- **ワークスペース戦略:** 1 State = 1 Workspace。
    - Terraform Cloudワークスペース名（現行 `aws-admin-cmn`）を本番アカウント単体運用向けに見直すか否かは<mark>未確定（Milestone `03.構築` のIssueで確定する）</mark>。
- **変数管理:**
